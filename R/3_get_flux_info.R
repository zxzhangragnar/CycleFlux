...
get_basic_gene_info <- function(input_net_file, output_path, res_path, input_tumor_name, input_tumor_data, input_normal_data) {
  ...
  source(system.file("rscript/3_flux_subnet/1_funcs_tool/1_get_gene_foldchange.R", package = "CycleFlux"), local=ENV)
  ENV$get_gene_foldchange_main(output_path, res_path, input_tumor_name, input_tumor_data, input_normal_data)

  source(system.file("rscript/3_flux_subnet/1_funcs_tool/2_get_gene_and_tofind_list.R", package = "CycleFlux"), local=ENV)
  ENV$get_gene_and_tofind_list_main(output_path, res_path, package_path)

  source(system.file("rscript/3_flux_subnet/1_funcs_tool/3_get_gene_count.R", package = "CycleFlux"), local=ENV)
  ENV$get_gene_count_main(output_path, res_path, input_tumor_name, input_tumor_data, input_normal_data)
