      ...
      for (j in 1:length(temp_part_cyc_df[,1])) {
        if (temp_part_cyc_df[j,"ifgap"] == "gap") {
          ...
          if(length(other_down_rct_df[,1]) == 0) {
            ...
            other_normal_lowest_val = min(other_down_rct_df$mean_fc)
          }
          ...
          if(length(other_up_rct_df[,1]) == 0) {
            ...
            other_up_highest_val = max(other_up_rct_df$mean_fc)
            other_up_highest_rct = other_up_rct_df[other_up_rct_df$mean_fc == other_up_highest_val,"rid"]
            if (length(other_up_highest_rct) > 1) {
              other_up_highest_rct = other_up_highest_rct[1]
            }
            other_up_every_val = mean(other_up_rct_df$mean_fc)  
          }
          
          
          other_all_every_val = mean(temp_other_part_cyc_df$mean_fc)
