...
build_net <- function(hsa_net) {
  ...
  for (i in 1:length(hsa_net[,1])) {
    ...
    rid = hsa_net[i, "Rid"]
    enzyme = hsa_net[i, "EC"]
    pathway = hsa_net[i, "Pathway"]
    gene_symbol = hsa_net[i, "Gene_symbol"]
    
    if (!are.connected(g, c_in, c_out)) {
      g <- add_edges(g, c(c_in, c_out), 
                     rid = rid,
                     enzyme = enzyme,
                     pathway = pathway,
...
get_template_cm <- function(hsa_net, connected_compounds) {
  ...
  for (i in 1:length(connected_net[,1])) {
    temp_cin = connected_net[i, "C_in"]
    temp_cout = connected_net[i, "C_out"]
...
find_directed_cycle <- function(hsa_net, g) {
  ...
  connected_compounds = compound_components[[1]]
...
get_express_string <- function(cycle_var) {
  ...
  cycle_var_arr = c()
  for (j in 1:length(cycle_var_table[,1])) {
...
get_expression_main <- function(input_net_file, output_path) {
  ...
  save(g, file=file.path(output_path, "g.RData"))
  save(cycle_directed, file=file.path(output_path, "cycle_directed.RData"))
  save(subnet_edge_expression, file=file.path(output_path, "subnet_edge_expression.RData"))
  save(cycle_edge_expression, file=file.path(output_path, "cycle_edge_expression.RData"))
  save(cycle_expression, file=file.path(output_path, "cycle_expression.RData"))
  save(cycle_edgesucs_expression_in, file=file.path(output_path, "cycle_edgesucs_expression_in.RData"))
  save(cycle_edgesucs_expression_out, file=file.path(output_path, "cycle_edgesucs_expression_out.RData"))
}
