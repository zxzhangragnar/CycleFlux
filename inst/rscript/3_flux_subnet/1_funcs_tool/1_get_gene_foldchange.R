##############################################################
# 目的: 读入TCGA.RData源文件
# 生成gene_foldchange_list
# 存放 每个基因计算得到的FoldChange信息
#
##############################################################

# from teacher: old_test_tcga_gene.R
...
# mean_t = c(12,18,24,30,42)
# mean_n = c(4,6,8,10,13)
# fc <- foldchange(mean_t,mean_n)
# fc = c(1,1,1,1)
# log_Fold_change = foldchange2logratio(fc, base = 2)

#log2(mean(case))- log2(mean(control))或log2(mean(case)/mean(control))
#https://www.cnblogs.com/chenwenyan/p/13602734.html
