####################################################################################################
# gene_missing_list: 找到在tcga中缺失的gene

# 结合 to_find_correct_symbol 数据，第一列kegg基因对应第二列TCGA基因
# 先执行此函数,再执行3_get_cyc_info_1.R
####################################################################################################
...
####################################################################################################
#环中有的基因在to_find_correct_symbol的1 2列中的有哪些
#tool function
get_cyc_gene_not_in_tgca<-function(cyc_gene_expressinfo, gene_foldchange_list, to_find_correct_symbol_v1_1){
  ...
  #记录还有哪些环中的基因在 statr(tcga)数据中是找不到的
  cyc_gene_not_in_tgca = c()
  cyc_gene_in_tgca = c()
  #tcga_gene_list = rownames(statr)
...
get_gene_and_tofind_list_main <- function(output_path, res_path, package_path) {
  ...
  #save(gene_missing_list, file="E:/scFEA_universal/my_R/aimA/rdata_cycle_detect/3_flux_subnet/result_tool/gene_missing_list.RData")
