...
get_cycle_sum_stat <- function(tumor_name, all_gene_stat, cycle_expression, gene_missing_list)
{
  ...
  cycle_stat = as.data.frame(cbind(cycle_expression[,"cycle_id"],0))
  for (i in 1:length(cycle_expression[,"cycle_id"])) {
  ...
  colnames(cycle_stat) = c("cycle_id", "updownsign")
...
merge_struct<-function(tumor_name, cycle_merge_stat, struct_cycle_sort_list) {
  #struct
  temp_tumor_df = cycle_merge_stat[[tumor_name]]
  temp_tumor_df[which(temp_tumor_df$cycle_id %in% struct_cycle_sort_list[["net"]]),"stcid"] = 1
  temp_tumor_df[which(temp_tumor_df$cycle_id %in% struct_cycle_sort_list[["hub"]]),"stcid"] = 2
  temp_tumor_df[which(temp_tumor_df$cycle_id %in% struct_cycle_sort_list[["idv"]]),"stcid"] = 3
  temp_tumor_df[which(temp_tumor_df$cycle_id %in% struct_cycle_sort_list[["nrl"]]),"stcid"] = 4
  return(temp_tumor_df)
}
